...
public class Tabuleiro {
	...
	public int preencher(int posicaoX, int valor) throws colunaOcupadaException { //esse método encontra uma posição vazia
		...
		while (linhaAux < x && this.tab[linhaAux][posicaoX] == 0) { //o while é utilizado para incrementar a linha auxilixar que verifica se a próxima linha está vazia ou não
			linhaAux++;
		}
	...
	public String toString() { //o toString aqui printa o tabuleiro bonitinho
		...
		for (int linha = 0; linha < x; linha++) {
			str += "|---.---.---.---.---.---.---|\n"; //linhas fixas
			for (int coluna = 0; coluna < y; coluna++) {
				if(this.tab[linha][coluna] == 0) { //o if é para quando o espaço tiver vazio
					str += "|   ";
				}
				else {
					str += "| " +this.tab[linha][coluna]+ " "; //o else é para quando tiver algum número printado (1 ou 2 no caso)
				}
			}
			str += "|\n";
		}
	...
	return 0;
	 ...
	 public int verificarDiagonal(int posicaoX, int posicaoY) {
		 ...
		 while(linha < x && coluna < y && this.tab[linha][coluna] == this.tab[posicaoX][posicaoY]) {
			 j++;
			 linha++;
			 coluna++;
		 }
		 ...
		 while(linha >= 0 && coluna >= 0 && this.tab[linha][coluna] == this.tab[posicaoX][posicaoY]) {
			 j++;
			 linha--;
			 coluna--;
		 }
		 ...
		 if(j>= 4) {
			 return this.tab[posicaoX][posicaoY];
			 }
		 ...
		 while(linha < x && coluna >= 0 && this.tab[linha][coluna] == this.tab[posicaoX][posicaoY]) {
			 j++;
			 linha++;
			 coluna--;
		 }
		 ...
		 while(linha >= 0 && coluna < y && this.tab[linha][coluna] == this.tab[posicaoX][posicaoY]) {
			 j++;
			 linha--;
			 coluna++;
		 }
